...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        const { name, content, attachment_url, attachment_name, attachment_type } = req.body;
        if (!content && !attachment_url) return res.status(400).json({ error: 'Content or file required' });

        try {
            await sql`
        INSERT INTO posts (thread_id, name, content, attachment_url, attachment_name, attachment_type) 
        VALUES (${id}, ${name || 'Lucky Guest'}, ${content || ''}, ${attachment_url || null}, ${attachment_name || null}, ${attachment_type || null})
      `;
